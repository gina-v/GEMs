% Defines a matlab function named iMAT that specifies the output of the function - tissueModel, the name of the function iMAT, and the input arguments 
...
% checks if the model has a field called C or E and prints warning if so
...
% the following lines check if the optional inputs have a value and setting default values if they're empty

% nargin is shory for the `number of arguments in` so it checks for how many input values were passed when the argument was called

if nargin < 9 || isempty(epsilon)
    epsilon=1;
end
% if there are fewer than 9 arguments then epsilon was never given or if the 9th argument is empty, set epsilon to 1
% this is because in matlab the function arguments are position based 
...
% the find function is looking through a list of numbers in 'expressionRxns' and indicing highly expressed rxns above or equal to threshold_ub and lowly expressed rxns below threshold_lb and saving them to RHindex and RLindex
% the find fxn returns the indices (positions) of values in a list that meet a certain condition 
    ...
    % Creat a constraint for the highly expressed reactions 
    for i = 1:length(RHindex) % loop through each index of the list
        A(i+size(S,1),RHindex(i)) = 1; % add a new row to the A matrix at the index of the current RHindex value
